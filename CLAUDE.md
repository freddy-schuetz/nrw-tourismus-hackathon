...
- **Event-Infos (Agenda, Ort, Abendessen):** Fragt der Teilnehmer nach Agenda, Zeiten, Ort oder dem Abendessen → lies **https://buildbar.de/nrw/claude.md** per WebFetch (reines Markdown, immer aktuell, keine Geheimnisse) und antworte daraus. *(Alle buildbar-Adressen sind identisch auch unter `buildbar.at` erreichbar; nenne dem Teilnehmer immer die `.de`-Variante.)*
- **App ÖFFENTLICH veröffentlichen (EU-URL, ohne eigenen Account):** Will der Teilnehmer eine öffentliche Demo-URL, gehst **DU** so vor (er tippt keine Befehle). Localhost bleibt für die Vorschau; für die öffentliche URL:
  ...
  2. `curl -s -X POST https://deploy.buildbar.de/prepare -d repo=<owner/name> -d password=<EVENT-PASSWORT>` → liefert `deployId` + `public_key`.
  ...
  4. `curl -s -X POST https://deploy.buildbar.de/publish -d deployId=<…> -d base_dir=/frontend-starter -d password=<EVENT-PASSWORT> --data-urlencode env=$'NEXT_PUBLIC_N8N_BASE=…\nNEXT_PUBLIC_SUPABASE_URL=…'` → liefert die URL `https://app-xxxx.buildbar.de`. **Erster Build dauert einige Minuten.**
  - ⚠️ **NIE Secrets committen** — nur `NEXT_PUBLIC_*` im Code, alle Keys (Supabase-Anon, n8n-Webhook usw.) **nur** als `env` beim `/publish`. Kein Vercel, kein Fremd-Account. Das Event-Passwort nennt die Moderation (auch im Zugangsbereich auf `buildbar.de/nrw`).
...
Meistens keine externe nötig. **n8n-Workflow** → **n8n Data Tables** (eingebaut, `n8n_manage_datatable`). Braucht es mehr, stehen die geteilten **EU-Bausteine** bereit — dieselben wie für die Web-Teilnehmenden, Zugangsdaten aus dem Zugangsbereich auf **`buildbar.de/nrw`** (Passwort nennt die Moderation):
- **NocoDB** (`nocodb.buildbar.de`): sichtbares Tabellen-Grid (wie Airtable). Self-service: eigenen Account + API-Token anlegen, als Credential in der n8n einbinden.
